tableextension 50000 "Customer Extension" extends Customer
{
  fields
  {
    modify(Name)
    {
    trigger OnAfterValidate()var contact: Record contact;
    begin
      if rec.name <> xRec.name then begin
        if "Primary Contact No." = '' then rec.validate(contact, name)
        else
        begin
    ...
    field(50000;NRIC;Text[20])
    {
      ...
      trigger OnValidate()var contact: Record contact;
      begin
        if rec.NRIC <> xRec.NRIC then begin
          if "Primary Contact No." <> '' then if contact.get("Primary Contact No.")then begin
    ...
    field(50016;"Overdue Days";Integer)
    {
      ...
      CalcFormula = Count("Booking Entry" WHERE("Driver No."=FIELD("No."), "Posting Date"=field("Date Filter")));
      Editable = false;
      Caption = 'Overdue Days';
    }
    field(50017;"Overdue Amount";Decimal)
    {
      CalcFormula = Sum("Booking Entry".Amount WHERE("Driver No."=FIELD("No."), "Posting Date"=field("Date Filter")));
      Editable = false;
      FieldClass = FlowField;
