page 50033 "CM Pass Card"
{
        ...
        area(content)
        {
            group("Pass Card")
            {
                field("Serial No."; Rec."Serial No.")
                {
                    ...
                    var
                        PassCardrec: Record "Pass Card";
                        PasscardList: page "Pass Card List";
                    begin
                        clear(PAsscardrec);
                        passcardrec.reset;
                        if g_ContractNo <> '' then
                            passcardrec.SetRange("Contract No.", g_ContractNo);
                        passcardList.SetRecord(passcardRec);
                        passcardList.SetTableView(passcardRec);
                        passcardList.LookupMode := true;
                        if passcardlist.RunModal() = action::LookupOK then begin
                            passcardList.GetRecord(passcardRec);
                            validate("Serial No.", PassCardrec."Serial No.");
                        end;
                    end;*/
                }
    ...
    procedure GetContract(_ContractNo: Code[10])
    begin
        g_ContractNo := _ContractNo;
    end;


    var
        g_ContractNo: Code[10];
