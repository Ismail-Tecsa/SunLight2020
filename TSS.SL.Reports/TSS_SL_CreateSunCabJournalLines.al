report 50000 "Create SunCab Journal Lines"
{

    ApplicationArea = Basic, Suite;
    Caption = 'Create General Journal Lines';
    ProcessingOnly = true;
    UsageCategory = Tasks;

    dataset
    {
        dataitem(SunCabGenJournalLine; "SunCab Gen. Journal Line")
        {
            ...
            var
                ...
                SunBank: record "SunCab Bank Setup";
            begin
                GenJnlLine.Init();
                ...
                if PostingDate <> 0D then
                    GenJnlLine.Validate("Posting Date", SunCabGenJournalLine."Posting Date");

                GenJnlLine.Validate("Document Type", SunCabGenJournalLine."Document Type");
                if (GenJnlLine."Document No." = '') and (SunCabGenJournalLine."Document No." <> '') then
                    GenJnlLine.Validate("Document No.", SunCabGenJournalLine."Document No.");

                SunBank.get(SunCabGenJournalLine."Bal. Account No.");

                GenJnlLine.Validate("Account Type", SunBank."Account Type");
                GenJnlLine.Validate("Account No.", sunbank."Account No.");
                GenJnlLine.validate("Bal. Account Type", "Bal. Account Type"::"Bank Account");
                genjnlline.Validate("Bal. Account No.", SunBank."SunCab Bank");

                GenJnlLine.Validate("Posting Date", SunCabGenJournalLine."Posting Date");
                GenJnlLine."Posting Date" := SunCabGenJournalLine."Posting Date";

                genjnlline.validate(amount, -SunCabGenJournalLine.amount);
