table 50025 "Settlement Header"
{
    ...
    fields
    {
            ...
            begin
                ...
                if not SetLines.findset then begin
                    if confirm('Would you like to auto create the lines?') then begin
                        ...
                        setmaster.Findfirst();
                        repeat
                            if (SetMaster."Settlement Type" = SetMaster."Settlement Type"::" ") or
                            (SetMaster."Settlement Type" = "Settlement Type") then begin
        ...
        field(26; "Cutoff Overdue Days"; Integer)
        {
            ...
            CalcFormula = Count("Booking Entry" WHERE("Taxi No." = FIELD("Taxi ID"),
                                                     "Posting Date" = field("Cut off Date Filter")));
            Editable = false;
            Caption = 'Cutoff Overdue Days';
        }
        field(27; "Cutoff Overdue Amount"; Decimal)
        {
            CalcFormula = Sum("Booking Entry".Amount WHERE("Taxi No." = FIELD("Taxi ID"),
                                                    "Posting Date" = field("Cut off Date Filter")));
            Editable = false;
            FieldClass = FlowField;
