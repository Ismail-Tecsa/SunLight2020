table 50015 Case
{
    ...
    fields
    {
        ...
        field(4; "Taxi ID"; Code[10])
        {
            ...
            begin
                taxis.SetRange("Taxi ID", "Taxi ID");
                If taxis.FindFirst() then
                    "Permit ID" := taxis."Permit ID";
                //TSA_ISMAIL Cmacc uodate
                if Type = type::Accident then begin
                    taxis.SetRange("Taxi ID", "Taxi ID");
                    If taxis.FindSet() then begin
                        Insurance.reset;
        ...
        field(7; "Contract No."; code[10])
        {
            ...
            begin
                if ContractHead.get(ContractHead."Contract Type"::Contract, "Contract No.") then begin
                    ...
                    //TSA_ISMAIL
                    If Type = Type::"Pass Card" then begin
                        Passcard.SetRange("Taxi ID", "Taxi ID");
                        if passcard.FindFirst() then begin
                            CMPassCard.Get("Case ID");
                            CMPassCard."Serial No." := passcard."Serial No.";
                            CMPassCard."Start Date" := passcard."Start Date";
                            CMPassCard."End Date" := passcard."Expiry Date";
                            CMPassCard.Modify;
                        end;
                    end;
                    //TSA_ISMAIL
        ...
        field(2; Type; Option)
        {
            ...
            trigger OnValidate()
            var
                CMLostDoc: record "CM Lost Document";
            ...
            begin
                case xRec.type of
                    xrec.type::"Lost Document":
                        begin
                            if CMLostDoc.get("Case ID") then
                                if Confirm('Are you sure you want to delete the existing type details?') then begin
                                    CMLostDoc.delete;
                                end;
                        end;
                    xrec.type::"Endorse Engine":
                        begin
                            if CMEndEng.get("Case ID") then
                                if Confirm('Are you sure you want to delete the existing type details?') then begin
                                    CMEndEng.delete;
                                end;
                        end;
                    xrec.type::"Pass Card":
                        begin
                            if CMPassCard.get("Case ID") then
                                if Confirm('Are you sure you want to delete the existing type details?') then begin
                                    CMPassCard.delete;
                                end;
                        end;
                    ...
                    xrec.type::Accident:
                        begin
                            if CMAccident.get("Case ID") then
                                if Confirm('Are you sure you want to delete the existing type details?') then begin
                                    CMAccident.delete;
                                    CMAccOffer.reset;
                    ...
                    xrec.type::Dismantle:
                        begin
                            if CMDismantle.get("Case ID") then
                                if Confirm('Are you sure you want to delete the existing type details?') then begin
                                    CMDismantle.delete;
                                end;
                        end;
