table 50020 "CM Pass Card"
{
    ...
    fields
    {
        ...
        field(2; "Serial No."; Text[30])
        {
            ...
            var
                ...
                Casemana: record "Case";
                constractNo: Code[10];
            begin
                If Casemana.Get("Case ID") then begin
                    constractNo := Casemana."Contract No.";
                    Message('%1', constractNo);
                    PassCard.Reset();
                    PassCard.SetRange("Contract No.", constractNo);
                    if Page.RunModal(0, PassCard) = Action::LookupOK then begin
                        "Serial No." := PassCard."Serial No.";
                        Message('%1', "Serial No.");
                    end;
                end;
            end;*/
            ...
            var
                PassCard: Record "Pass Card";
            begin
                if PassCard.get("Serial No.") then begin
                    "Start Date" := PassCard."Start Date";
                    "End Date" := PassCard."Expiry Date";
                    //"Serial No." := PassCard."Serial No.";//TSA_ISMAIL
                end;
            end;*/
        }
    ...
    begin
        if "Form APAD" and Permit and "Driver IC" and "Driver License" and "Driver PSV" and "Permit Company Auth. Letter" and "Signatories IC" and "Takeout Form" then begin
            if "Document on Hand Date" <> 0D then
                "Document on Hand Date" := today;
        end else begin
            if "Document on Hand Date" <> 0D then
                "Document on Hand Date" := 0D;
        end
    end;
